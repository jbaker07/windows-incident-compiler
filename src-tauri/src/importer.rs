...
impl SafeImporter {
                ...
                if size > self.limits.max_single_file_bytes {
                    rejected.push(RejectedFile {
                        ...
                        message: format!(
                            ...
                            size, self.limits.max_single_file_bytes
                        ),
                    });
                    continue;
                }
    ...
    fn import_zip(&self, source: &Path) -> Result<ImportResult, String> {
        ...
        let file = File::open(source).map_err(|e| format!("Failed to open zip: {}", e))?;
        let mut archive =
        ...
        for i in 0..archive.len() {
            ...
            if uncompressed_size > self.limits.max_single_file_bytes {
                rejected_files.push(RejectedFile {
                    ...
                    message: format!(
                        ...
                        uncompressed_size, self.limits.max_single_file_bytes
                    ),
                });
                continue;
            }
            ...
            if compressed_size > 0 {
                ...
                if ratio > self.limits.max_compression_ratio {
                    rejected_files.push(RejectedFile {
                        ...
                        message: format!(
                            "Compression ratio {:.1} exceeds limit {}",
            ...
            fs::write(&dest_path, &content).map_err(|e| format!("Failed to write file: {}", e))?;
    ...
    fn validate_zip_archive(
        ...
        if file_count > self.limits.max_files {
            return Ok((
                false,
                format!("Too many files: {} > {}", file_count, self.limits.max_files),
            ));
        }
    ...
    fn build_summary(
        ...
        let events_extracted = files.iter().filter_map(|f| f.events_extracted).sum();
