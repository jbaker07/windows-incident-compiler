...
fn test_import_folder_produces_manifest() {
    ...
    assert!(
        ...
        "Bundle ID should not be empty"
    );
    assert!(
        result.summary.total_files >= 5,
    ...
    assert!(
        manifest.get("bundle_id").is_some(),
        "Manifest should have bundle_id"
    );
    assert!(
    ...
    assert!(
        manifest.get("summary").is_some(),
        "Manifest should have summary"
    );
...
fn test_import_validates_file_types() {
    ...
    let manifest_content =
        fs::read_to_string(&result.manifest_path).expect("Failed to read manifest");
    ...
    assert!(
        kinds.iter().any(|k| k.contains("json")
            || k.contains("xml")
            || k.contains("text")
            || *k == "unknown"),
        "Expected various file kinds, got: {:?}",
        kinds
    );
