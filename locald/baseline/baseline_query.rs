...
impl BaselineQuery {
    ...
    pub fn is_new_remote_ip(baseline: &HostBaseline, exe_key: &str, remote_ip: &str) -> (bool, usize) {
    ...
    pub fn is_new_identity_activity(baseline: &HostBaseline, identity_key: &str, tag: &str) -> (bool, usize) {
        if let Some(tags) = baseline.first_seen.activity_tags_per_identity.get(identity_key) {
    ...
    pub fn is_burst(baseline: &HostBaseline, metric_name: &str, observed_rate: f64, threshold_k: f64, min_floor: f64) -> (bool, f64) {
        if let Some(rate_baseline) = baseline.rate_baselines.get(metric_name) {
            ...
            let threshold = (rate_baseline.ema + threshold_k * rate_baseline.ema_var.sqrt()).max(min_floor);
            (is_burst, threshold)
        } else {
    ...
    pub fn get_feature_vector_stats(baseline: &HostBaseline, vector_type: &str) -> Option<(f64, f64, u32)> {
        baseline.feature_vector_baselines.get(vector_type).map(|fv| {
            let mean_mean = fv.means.values().sum::<f64>() / (fv.means.len() as f64 + 1e-6);
            let mean_var = fv.variances.values().sum::<f64>() / (fv.variances.len() as f64 + 1e-6);
...
mod tests {
    ...
    fn test_is_new_remote_ip() {
        ...
        baseline.first_seen.remote_ips_per_exe.insert(
            "exe1".to_string(),
            ["192.168.1.1", "192.168.1.2"].iter().map(|s| s.to_string()).collect(),
        );
    ...
    fn test_is_new_parent_child() {
        ...
        assert!(!BaselineQuery::is_new_parent_child(&baseline, "parent", "child1"));
        assert!(BaselineQuery::is_new_parent_child(&baseline, "parent", "child3"));
        assert!(BaselineQuery::is_new_parent_child(&baseline, "unknown_parent", "child1"));
    }
    ...
    fn test_is_burst() {
        ...
        // Manually add a rate baseline
        let mut rate_bl = super::super::types::RateBaseline::new();
        rate_bl.update(5.0, 0.1, 0);
        rate_bl.update(5.0, 0.1, 1);
        baseline.rate_baselines.insert("metric1".to_string(), rate_bl);

        let (is_burst, _threshold) = BaselineQuery::is_burst(&baseline, "metric1", 6.0, 2.0, 1.0);
        assert!(!is_burst);

        let (is_burst, _threshold) = BaselineQuery::is_burst(&baseline, "metric1", 50.0, 2.0, 1.0);
        assert!(is_burst);
    }
}
