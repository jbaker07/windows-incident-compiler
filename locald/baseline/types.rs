...
impl FeatureVectorBaseline {
    ...
    pub fn distance(&self, features: &HashMap<String, f64>) -> f64 {
        ...
        let mut dist_sq = 0.0;
        for (feature_name, value) in features {
            if let (Some(mean), Some(var)) = (self.means.get(feature_name), self.variances.get(feature_name)) {
...
mod tests {
    ...
    fn test_rate_baseline_burst() {
        let mut baseline = RateBaseline::new();
        baseline.update(5.0, 0.1, 0);
        baseline.update(5.0, 0.1, 1);
        baseline.update(5.0, 0.1, 2);
        
        // Normal rate should not trigger
        assert!(!baseline.is_burst(6.0, 2.0, 1.0));
        
        // Spike should trigger
        assert!(baseline.is_burst(50.0, 2.0, 1.0));
    }
