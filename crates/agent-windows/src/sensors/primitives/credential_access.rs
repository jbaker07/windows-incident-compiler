...
pub fn detect_cred_access(base_event: &Event) -> Option<Event> {
    let cred_tools = [
        "lsass.exe", "procdump.exe", "procdump", "minidump.exe",
        "mimikatz", "mimikatz.exe", "Invoke-Mimikatz",
        "powershell.exe", "powershell",  // With credential patterns (filtered below)
        "wmic.exe", "wmic",
        "tasklist.exe", "tasklist",  // Used in credential enumeration chains
    ];
    ...
    let matched_tool = cred_tools.iter().find(|tool| {
    ...
    fields.insert(event_keys::CRED_TOOL.to_string(), json!(matched_tool.to_string()));
    ...
    Some(Event {
        ...
        tags: vec!["windows".to_string(), "credential_access".to_string(), "sysmon".to_string()],
