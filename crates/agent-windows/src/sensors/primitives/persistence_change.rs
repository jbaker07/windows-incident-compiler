...
const REGISTRY_PERSISTENCE: &[(&str, &str)] = &[
    // Run keys
    ("\\Software\\Microsoft\\Windows\\CurrentVersion\\Run", "registry_run"),
    ("\\Software\\Microsoft\\Windows\\CurrentVersion\\RunOnce", "registry_run"),
    ("\\Software\\Microsoft\\Windows\\CurrentVersion\\RunServices", "registry_run"),
    ("\\Software\\Microsoft\\Windows\\CurrentVersion\\RunServicesOnce", "registry_run"),
    ("\\Software\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Run", "registry_run"),
    // AppInit_DLLs
    ...
    ("\\Software\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon\\Notify", "winlogon"),
    // Active Setup
    ("\\Software\\Microsoft\\Active Setup\\Installed Components", "active_setup"),
    ...
    ("\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\ShellExecuteHooks", "shell_extension"),
    ("\\Software\\Microsoft\\Windows\\CurrentVersion\\ShellServiceObjectDelayLoad", "shell_extension"),
    // Browser extensions
    ("\\Software\\Microsoft\\Internet Explorer\\Extensions", "browser_extension"),
];

/// File-based persistence locations (Startup folders)
const FILE_PERSISTENCE: &[(&str, &str)] = &[
    ("\\Start Menu\\Programs\\Startup\\", "startup_folder"),
    ("\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\", "startup_folder"),
    ("ProgramData\\Microsoft\\Windows\\Start Menu\\Programs\\StartUp", "startup_folder"),
];
...
pub fn detect_persistence_change(base_event: &Event) -> Option<Event> {
    ...
    if !matches!(event_id, 12 | 13 | 14) && !base_event.tags.iter().any(|t| t.contains("registry")) {
        return None;
    }

    // Get registry path
    let target_object = base_event.fields
        .get("TargetObject")
    ...
    fields.insert(event_keys::PERSIST_LOCATION.to_string(), json!(target_object));
    ...
    Some(Event {
        ...
        tags: vec!["windows".to_string(), "persistence_change".to_string(), "sysmon".to_string()],
...
pub fn detect_persistence_from_file(base_event: &Event) -> Option<Event> {
    ...
    let (persist_type, _) = FILE_PERSISTENCE.iter()
    ...
    fields.insert(event_keys::PROC_EUID.to_string(), json!(user));
    fields.insert(event_keys::PROC_EXE.to_string(), json!(image));
    fields.insert(event_keys::PERSIST_LOCATION.to_string(), json!(target_filename));
    fields.insert(event_keys::PERSIST_TYPE.to_string(), json!(persist_type));
...
pub fn detect_persistence_from_task(base_event: &Event) -> Option<Event> {
    ...
    let task_name = base_event.fields
        .get("TaskName")
        .and_then(|v| v.as_str())?;
    ...
    fields.insert(event_keys::PERSIST_TYPE.to_string(), json!("scheduled_task"));
    fields.insert(event_keys::PERSIST_ACTION.to_string(), json!(action));

    // Include task content if available
    if let Some(content) = base_event.fields.get("TaskContent").and_then(|v| v.as_str()) {
    ...
    Some(Event {
        ts_ms: base_event.ts_ms,
        host: base_event.host.clone(),
        tags: vec!["windows".to_string(), "persistence_change".to_string(), "security".to_string()],
        proc_key: base_event.proc_key.clone(),
        file_key: None,
        identity_key: base_event.identity_key.clone(),
        evidence_ptr: None,
...
pub fn detect_persistence_from_service(base_event: &Event) -> Option<Event> {
    ...
    let service_name = base_event.fields
        .get("ServiceName")
        .or_else(|| base_event.fields.get("TargetServiceName"))
        .and_then(|v| v.as_str())?;

    let service_file = base_event.fields
        .get("ServiceFileName")
    ...
    fields.insert(event_keys::PROC_UID.to_string(), json!(user.clone()));
    fields.insert(event_keys::PROC_EUID.to_string(), json!(user));
    fields.insert(event_keys::PROC_EXE.to_string(), json!(service_file.unwrap_or("unknown")));
    fields.insert(event_keys::PERSIST_LOCATION.to_string(), json!(service_name));
