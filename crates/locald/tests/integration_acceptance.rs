...
use edr_locald::integrations::{
    ...
    export::{ExportedIncident, JoinKeys, TimelineFactSummary, EvidenceSummary, ExportMetadata},
    ingest::{WazuhAlertsSource, ZeekEveSource, IngestSource},
    ...
    let exported = ExportedIncident {
        ...
        timeline_summary: vec![
            TimelineFactSummary {
                ...
                key_fields: HashMap::from([
                    ("exe".to_string(), "mimikatz.exe".to_string()),
                ]),
            }
        ],
...
fn test_ingest_wazuh_alert_produces_stable_scope_key() {
    ...
    assert!(key_str.contains("vendor_alert"), "Scope key should contain 'vendor_alert': {}", key_str);
}
...
fn test_ingest_zeek_eve_dns_produces_fact_with_query() {
    ...
    assert_eq!(alert.dns_query, Some("malicious-c2.example.com".to_string()));
    
    // Verify IP extraction
    assert!(alert.ip_indicators.len() >= 2, "Expected at least 2 IP indicators");
    let src_ip = alert.ip_indicators.iter().find(|i| i.direction == IpDirection::Source);
    let dst_ip = alert.ip_indicators.iter().find(|i| i.direction == IpDirection::Destination);
...
fn test_vendor_alert_soft_join_by_ip() {
    ...
    let joinable_ips = alert.ip_indicators.iter()
    ...
    let create_incident = || ExportedIncident {
        ...
        timeline_summary: vec![
            TimelineFactSummary {
                ...
                key_fields: HashMap::from([
                    ("path".to_string(), "/etc/cron.d/malware".to_string()),
                ]),
            },
        ],
        evidence_summary: vec![],
...
fn test_metrics_eqs_calculation() {
    ...
    let expected_eqs: f64 = 0.25 * timeline_score 
                         + 0.35 * evidence_score 
                         + 0.20 * entity_score 
                         + 0.20 * lineage_score;
...
fn test_tier0_families_are_defined() {
    ...
    for family in &tier0_families {
        ...
        let is_tier0 = match *family {
            "memory_rwx" | "lsass" | "credential_access" | "rootkit" => true,
            _ => false,
        };
