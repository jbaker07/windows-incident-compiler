...
pub enum FactType {
    ...
    /// Used for credential access detection (e.g., LSASS access)
    ProcessAccess {
        /// Process performing the access
        source_proc_key: String,
        /// Process being accessed
        target_proc_key: String,
        /// Target image path (e.g., C:\Windows\System32\lsass.exe)
        target_image: String,
        /// Granted access mask (hex string, e.g., "0x1410")
        granted_access: String,
        /// Call trace (optional, for determining source module)
...
impl Fact {
    ...
    pub fn domain(&self) -> FactDomain {
        match &self.fact_type {
            ...
            FactType::ProcessAccess { .. } => FactDomain::Auth,  // credential access attempts
