...
use crate::slot_matcher::{
    CapabilityLevel, CapabilityRegistry, PlaybookDef, PlaybookSlot, SlotPredicate,
};
...
fn make_exec_fact(host_id: &str, exe_path: &str, scope_key: &str) -> Fact {
    ...
    Fact::new(
        host_id,
        ScopeKey::Process { key: scope_key.to_string() },
        FactType::Exec {
...
mod tests {
    ...
    fn test_two_required_slots_fire() {
        ...
        assert!(!affected1.is_empty(), "Exec fact should create/update hypothesis");
        ...
        assert!(!active[0].all_required_satisfied(), "Should NOT have all required slots filled");
    ...
    fn test_optional_slot_enriches() {
        ...
        playbook.slots.push(PlaybookSlot::optional(
            "dns_slot",
            "DNS Resolution",
            SlotPredicate::for_fact_type("DnsResolve"),
        ).with_ttl(300));
        ...
        if let Some(hyp) = controller.get_hypothesis(hyp_id) {
            ...
            assert!(!hyp.all_required_satisfied(), "Should NOT have all required slots (still missing connect)");
            
            // But the optional DNS slot should have been filled
            let dns_filled = hyp.slot_fills.contains_key("dns_slot");
            assert!(dns_filled, "Optional DNS slot should be filled for enrichment");
        } else {
    ...
    fn test_soft_capability_gating_blocks_required() {
        ...
        let affected2 = controller.ingest_fact(dns_fact).unwrap();
        ...
        if let Some(hyp) = controller.get_hypothesis(&affected1[0]) {
            ...
            let dns_filled = hyp.slot_fills.get("dns_slot")
                .map(|f| f.satisfied)
                .unwrap_or(false);
            assert!(!dns_filled, "SOFT DNS should NOT fill hard required slot");
            assert!(!hyp.all_required_satisfied(), "Should NOT have all required satisfied");
        }
    }
    ...
    fn test_determinism() {
        ...
        assert_eq!(affected1, affected2, "Same facts should produce same hypothesis IDs");
    ...
    fn test_hypothesis_expiry() {
        ...
        assert!(!active_before.is_empty(), "Hypothesis should be active initially");
